...
public class Main {
  ...
  //
  /**
   * The session factory needed to access the database (see the CRUD methods below).
   */
  private static final SessionFactory factory;

  /**
   * The static section below creates a Hibernate Configuration object, based on the contents of the hibernate.cfg.xml
   * configuration file. (The file includes information to connect to the database entities must be persisted to.) The
   * configuration object is then used to create a session factory.
   * <p>
   * Note: A static block is executed only once, when the embedding class is loaded into the JVM. It can be thought
   * of as an anonymous static method that the class loader automatically calls upon loading the class.
   */
  static {
    try {
  ...
  /**
   * Creates in (i.e. inserts into) the database the specified product. The method automatically computes and set the ID
   * of the product object passed as an argument. Exercise 1.3.
   *
   * @param product the product to create in the database
   *
   * @return the ID of the newly-inserted product, or {@code -1} if the product could not be created
   */
   ...
   *
   * @return the product instance, or {@code null} if no product with the specified ID was found in the database
   */
   ...
   *
   * @return all the products in the database or {@code Collections.emptyList()} if there is no product
   */
  ...
  /**
   * Retrieves from the database the products than cost less than the specified price. Exercise 2.2.
   * <p>
   * The method dynamically creates a new HQL query every time it is called, using the price argument. The {@code
   * raiseProductPriceBy} method uses an alternate method, based on named queries.
   *
   * @return all the products in the database that cost less than the specified price or {@code Collections.emptyList()}
  ...
  /**
   * Raises the price of all the products in the database by the specified amount. Exercise 2.2.
   * <p>
   * The method use a parameterized named query. The query parameter is replaced by the method argument before
   * executing. Note: named queries are syntactically-checked and pre-compiled at startup time.
   *
